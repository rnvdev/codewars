...
function iqTest(numbers){
  ...
  for(let i = 0; i < separated.length; i++){ 
    let check = parseInt(separated[i]) % 2;
    check == 0 ? par.push(separated[i]) : impar.push(separated[i]);
  }
  let procuraIndexPar = separated.indexOf(par[0]);
  let procuraIndexImpar = separated.indexOf(impar[0]);
  return impar.length > par.length ? procuraIndexPar + 1 : procuraIndexImpar + 1;
}

console.log(iqTest("3 2 3 3"));